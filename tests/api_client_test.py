...
def _get_keycloak_user_fixture(
    ...
    return WriteKeycloakUser(
        ...
        attributes={"some_attrib": f"val{suffix}"},
    )


@pytest.mark.vcr()
def test_search_for_existing_user():
    assert _keycloak_api_client_factory().search_users(query="testname") == [
        ReadKeycloakUser(
...
def test_get_not_existing_user_when_partially_matching_emails_returned():
    assert (
        _keycloak_api_client_factory().get_keycloak_user_by_email(
...
def test_get_not_existing_user_by_id():
    assert (
        _keycloak_api_client_factory().get_keycloak_user_by_id(
...
def test_register_then_update_then_get_user():
    ...
    ] == keycloak_api_client._get_user_identities(keycloak_id=keycloak_id)
...
def test_create_client_and_create_mapper():
    ...
    assert (
        _keycloak_api_client_factory().create_client(
            client_id=client_id, client_secret=client_secret, id=str(id_of_client)
        )
        is None
    )
...
def test_search_clients_by_client_id():
    ...
    assert (
        keycloak_api_client.search_clients_by_client_id(
